...
public class Tester {
	...
	case 1: 
	 System.out.println("Enter customer Name,email,password, customer type,registration amount,date(d-m-y): ");
           ...
           customer1.putIfAbsent(email,c1);
		  	  break;
	
	case 2:
	//for displaying use collection view
//		 for(Customer c : customer1.values())
//	        	System.out.println(c);	
		//customerlogin
	{
		System.out.println("Please enter your registered Email Id and Password");
			boolean ref=CustomerLogin(sc.next(),sc.next(), customer1);
			if(ref==true) {
	...
	//change password
	{
		System.out.println("Enter email id and old password and newpassword");
		boolean ref = ChangePassword(sc.next(),sc.next(),sc.next(), customer1);
		if(ref==true) {
	...
	//unsubscribe customer
	{
		...
		Customer remove = customer1.remove((sc.next()));
		if(remove==null) {
			System.out.println("Invalid email");
		}else {
			System.out.println("You are unsubscribe from your subscription");
		}
